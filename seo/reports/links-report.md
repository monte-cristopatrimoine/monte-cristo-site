# Contrôle des liens — Monte-Cristo Patrimoine
*Généré le 24/06/2026 à 19:44*

## Résumé

| | |
|---|---|
| Total liens analysés | **323** |
| Liens internes valides ✅ | **263** |
| Liens externes ⚪ | **31** |
| mailto / tel ⚪ | **29** |
| Liens cassés 🔴 | **0** |
| Pages futures inexistantes 🟠 | **0** |

> ✅ Aucun lien cassé ni page future inexistante détectés.

---
## Détail par page source

### `404.html`
| Lien | Catégorie | Note |
|------|-----------|------|
| `#main` | ✅ Ancre (même page) |  |
| `/` | ✅ Interne valide |  |
| `/` | ✅ Interne valide |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/blog` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/mentions-legales` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /mentions-legales |
| `/particuliers` | ✅ Interne valide |  |
| `/particuliers` | ✅ Interne valide |  |
| `/politique-confidentialite` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /politique-confidentialite |

### `article-cgp-independant.html`
| Lien | Catégorie | Note |
|------|-----------|------|
| `#main` | ✅ Ancre (même page) |  |
| `/` | ✅ Interne valide |  |
| `/` | ✅ Interne valide |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/blog` | ✅ Interne valide |  |
| `/blog` | ✅ Interne valide |  |
| `/blog` | ✅ Interne valide |  |
| `/conseiller-gestion-patrimoine-independant` | ✅ Interne valide |  |
| `/conseiller-gestion-patrimoine-independant` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet#modele` | ✅ Ancre (autre page) |  |
| `/le-cabinet#partenaires` | ✅ Ancre (autre page) |  |
| `/mentions-legales` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /mentions-legales |
| `/particuliers` | ✅ Interne valide |  |
| `/particuliers` | ✅ Interne valide |  |
| `/politique-confidentialite` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /politique-confidentialite |
| `https://www.linkedin.com/company/108398030/` | ⚪ Externe (non testé) |  |
| `https://www.orias.fr/search?number=25006472` | ⚪ Externe (non testé) |  |
| `mailto:kevin@monte-cristo.net` | ⚪ mailto / tel |  |
| `tel:+33629865266` | ⚪ mailto / tel |  |

### `article-frais-bancaires.html`
| Lien | Catégorie | Note |
|------|-----------|------|
| `#main` | ✅ Ancre (même page) |  |
| `/` | ✅ Interne valide |  |
| `/` | ✅ Interne valide |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/article-cgp-independant` | ✅ Interne valide |  |
| `/blog` | ✅ Interne valide |  |
| `/blog` | ✅ Interne valide |  |
| `/blog` | ✅ Interne valide |  |
| `/conseiller-gestion-patrimoine-independant` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet#modele` | ✅ Ancre (autre page) |  |
| `/le-cabinet#partenaires` | ✅ Ancre (autre page) |  |
| `/mentions-legales` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /mentions-legales |
| `/particuliers` | ✅ Interne valide |  |
| `/particuliers` | ✅ Interne valide |  |
| `/politique-confidentialite` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /politique-confidentialite |
| `/simulateur-frais` | ✅ Interne valide |  |
| `https://www.linkedin.com/company/108398030/` | ⚪ Externe (non testé) |  |
| `https://www.orias.fr/search?number=25006472` | ⚪ Externe (non testé) |  |
| `mailto:kevin@monte-cristo.net` | ⚪ mailto / tel |  |
| `tel:+33629865266` | ⚪ mailto / tel |  |

### `blog.html`
| Lien | Catégorie | Note |
|------|-----------|------|
| `#main` | ✅ Ancre (même page) |  |
| `/` | ✅ Interne valide |  |
| `/` | ✅ Interne valide |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/article-cgp-independant` | ✅ Interne valide |  |
| `/article-frais-bancaires` | ✅ Interne valide |  |
| `/blog` | ✅ Interne valide |  |
| `/blog` | ✅ Interne valide |  |
| `/conseiller-gestion-patrimoine-independant` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet#modele` | ✅ Ancre (autre page) |  |
| `/le-cabinet#partenaires` | ✅ Ancre (autre page) |  |
| `/mentions-legales` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /mentions-legales |
| `/particuliers` | ✅ Interne valide |  |
| `/particuliers` | ✅ Interne valide |  |
| `/politique-confidentialite` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /politique-confidentialite |
| `https://www.linkedin.com/company/108398030/` | ⚪ Externe (non testé) |  |
| `https://www.orias.fr/search?number=25006472` | ⚪ Externe (non testé) |  |
| `mailto:kevin@monte-cristo.net` | ⚪ mailto / tel |  |
| `tel:+33629865266` | ⚪ mailto / tel |  |

### `conseiller-gestion-patrimoine-independant.html`
| Lien | Catégorie | Note |
|------|-----------|------|
| `#main` | ✅ Ancre (même page) |  |
| `/` | ✅ Interne valide |  |
| `/` | ✅ Interne valide |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/blog` | ✅ Interne valide |  |
| `/blog` | ✅ Interne valide |  |
| `/conseiller-gestion-patrimoine-independant` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet#modele` | ✅ Ancre (autre page) |  |
| `/le-cabinet#modele` | ✅ Ancre (autre page) |  |
| `/le-cabinet#partenaires` | ✅ Ancre (autre page) |  |
| `/mentions-legales` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /mentions-legales |
| `/particuliers` | ✅ Interne valide |  |
| `/particuliers` | ✅ Interne valide |  |
| `/politique-confidentialite` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /politique-confidentialite |
| `https://www.linkedin.com/company/108398030/` | ⚪ Externe (non testé) |  |
| `https://www.orias.fr/search?number=25006472` | ⚪ Externe (non testé) |  |
| `mailto:kevin@monte-cristo.net` | ⚪ mailto / tel |  |
| `tel:+33629865266` | ⚪ mailto / tel |  |

### `entreprises.html`
| Lien | Catégorie | Note |
|------|-----------|------|
| `#main` | ✅ Ancre (même page) |  |
| `/` | ✅ Interne valide |  |
| `/` | ✅ Interne valide |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/blog` | ✅ Interne valide |  |
| `/blog` | ✅ Interne valide |  |
| `/conseiller-gestion-patrimoine-independant` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet#modele` | ✅ Ancre (autre page) |  |
| `/le-cabinet#partenaires` | ✅ Ancre (autre page) |  |
| `/mentions-legales` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /mentions-legales |
| `/particuliers` | ✅ Interne valide |  |
| `/particuliers` | ✅ Interne valide |  |
| `/politique-confidentialite` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /politique-confidentialite |
| `https://www.linkedin.com/company/108398030/` | ⚪ Externe (non testé) |  |
| `https://www.orias.fr/search?number=25006472` | ⚪ Externe (non testé) |  |
| `mailto:kevin@monte-cristo.net` | ⚪ mailto / tel |  |
| `tel:+33629865266` | ⚪ mailto / tel |  |

### `index.html`
| Lien | Catégorie | Note |
|------|-----------|------|
| `#contact` | ✅ Ancre (même page) |  |
| `#contact` | ✅ Ancre (même page) |  |
| `#contact` | ✅ Ancre (même page) |  |
| `#main` | ✅ Ancre (même page) |  |
| `/` | ✅ Interne valide |  |
| `/article-frais-bancaires` | ✅ Interne valide |  |
| `/blog` | ✅ Interne valide |  |
| `/blog` | ✅ Interne valide |  |
| `/blog` | ✅ Interne valide |  |
| `/conseiller-gestion-patrimoine-independant` | ✅ Interne valide |  |
| `/conseiller-gestion-patrimoine-independant` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet#modele` | ✅ Ancre (autre page) |  |
| `/le-cabinet#partenaires` | ✅ Ancre (autre page) |  |
| `/mentions-legales` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /mentions-legales |
| `/particuliers` | ✅ Interne valide |  |
| `/particuliers` | ✅ Interne valide |  |
| `/politique-confidentialite` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /politique-confidentialite |
| `https://www.linkedin.com/company/108398030/` | ⚪ Externe (non testé) |  |
| `https://www.orias.fr/search?number=25006472` | ⚪ Externe (non testé) |  |
| `mailto:kevin@monte-cristo.net` | ⚪ mailto / tel |  |
| `tel:+33629865266` | ⚪ mailto / tel |  |

### `le-cabinet.html`
| Lien | Catégorie | Note |
|------|-----------|------|
| `#main` | ✅ Ancre (même page) |  |
| `/` | ✅ Interne valide |  |
| `/` | ✅ Interne valide |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/blog` | ✅ Interne valide |  |
| `/blog` | ✅ Interne valide |  |
| `/conseiller-gestion-patrimoine-independant` | ✅ Interne valide |  |
| `/conseiller-gestion-patrimoine-independant` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet#modele` | ✅ Ancre (autre page) |  |
| `/le-cabinet#partenaires` | ✅ Ancre (autre page) |  |
| `/mentions-legales` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /mentions-legales |
| `/particuliers` | ✅ Interne valide |  |
| `/particuliers` | ✅ Interne valide |  |
| `/particuliers` | ✅ Interne valide |  |
| `/politique-confidentialite` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /politique-confidentialite |
| `https://www.linkedin.com/company/108398030/` | ⚪ Externe (non testé) |  |
| `https://www.orias.fr/search?number=25006472` | ⚪ Externe (non testé) |  |
| `mailto:kevin@monte-cristo.net` | ⚪ mailto / tel |  |
| `tel:+33629865266` | ⚪ mailto / tel |  |

### `mentions-legales.html`
| Lien | Catégorie | Note |
|------|-----------|------|
| `#article-1` | ✅ Ancre (même page) |  |
| `#article-10` | ✅ Ancre (même page) |  |
| `#article-2` | ✅ Ancre (même page) |  |
| `#article-3` | ✅ Ancre (même page) |  |
| `#article-4` | ✅ Ancre (même page) |  |
| `#article-5` | ✅ Ancre (même page) |  |
| `#article-6` | ✅ Ancre (même page) |  |
| `#article-7` | ✅ Ancre (même page) |  |
| `#article-8` | ✅ Ancre (même page) |  |
| `#article-9` | ✅ Ancre (même page) |  |
| `#main` | ✅ Ancre (même page) |  |
| `/` | ✅ Interne valide |  |
| `/` | ✅ Interne valide |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/blog` | ✅ Interne valide |  |
| `/blog` | ✅ Interne valide |  |
| `/conseiller-gestion-patrimoine-independant` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet#modele` | ✅ Ancre (autre page) |  |
| `/le-cabinet#partenaires` | ✅ Ancre (autre page) |  |
| `/mentions-legales` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /mentions-legales |
| `/particuliers` | ✅ Interne valide |  |
| `/particuliers` | ✅ Interne valide |  |
| `/politique-confidentialite` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /politique-confidentialite |
| `/politique-confidentialite` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /politique-confidentialite |
| `https://www.amf-france.org/` | ⚪ Externe (non testé) |  |
| `https://www.cmap.fr/consommateurs` | ⚪ Externe (non testé) |  |
| `https://www.cnil.fr` | ⚪ Externe (non testé) |  |
| `https://www.linkedin.com/company/108398030/` | ⚪ Externe (non testé) |  |
| `https://www.orias.fr` | ⚪ Externe (non testé) |  |
| `https://www.orias.fr` | ⚪ Externe (non testé) |  |
| `https://www.orias.fr/search?number=25006472` | ⚪ Externe (non testé) |  |
| `mailto:contact@monte-cristo.net` | ⚪ mailto / tel |  |
| `mailto:direction@monte-cristo.net` | ⚪ mailto / tel |  |
| `mailto:direction@monte-cristo.net` | ⚪ mailto / tel |  |
| `mailto:kevin@monte-cristo.net` | ⚪ mailto / tel |  |
| `tel:+33629865266` | ⚪ mailto / tel |  |
| `tel:+33677392890` | ⚪ mailto / tel |  |

### `mockup-contenu.html`
| Lien | Catégorie | Note |
|------|-----------|------|
| `#comparatif` | ✅ Ancre (même page) |  |
| `#entreprises` | ✅ Ancre (même page) |  |
| `#faq` | ✅ Ancre (même page) |  |
| `#modalites` | ✅ Ancre (même page) |  |
| `#schema` | ✅ Ancre (même page) |  |
| `#temoignages` | ✅ Ancre (même page) |  |

### `particuliers.html`
| Lien | Catégorie | Note |
|------|-----------|------|
| `#main` | ✅ Ancre (même page) |  |
| `/` | ✅ Interne valide |  |
| `/` | ✅ Interne valide |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/blog` | ✅ Interne valide |  |
| `/blog` | ✅ Interne valide |  |
| `/conseiller-gestion-patrimoine-independant` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet#modele` | ✅ Ancre (autre page) |  |
| `/le-cabinet#modele` | ✅ Ancre (autre page) |  |
| `/le-cabinet#partenaires` | ✅ Ancre (autre page) |  |
| `/mentions-legales` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /mentions-legales |
| `/particuliers` | ✅ Interne valide |  |
| `/particuliers` | ✅ Interne valide |  |
| `/politique-confidentialite` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /politique-confidentialite |
| `https://www.linkedin.com/company/108398030/` | ⚪ Externe (non testé) |  |
| `https://www.orias.fr/search?number=25006472` | ⚪ Externe (non testé) |  |
| `mailto:kevin@monte-cristo.net` | ⚪ mailto / tel |  |
| `tel:+33629865266` | ⚪ mailto / tel |  |

### `politique-confidentialite.html`
| Lien | Catégorie | Note |
|------|-----------|------|
| `#art-1` | ✅ Ancre (même page) |  |
| `#art-2` | ✅ Ancre (même page) |  |
| `#art-3` | ✅ Ancre (même page) |  |
| `#art-4` | ✅ Ancre (même page) |  |
| `#art-5` | ✅ Ancre (même page) |  |
| `#art-6` | ✅ Ancre (même page) |  |
| `#art-7` | ✅ Ancre (même page) |  |
| `#art-8` | ✅ Ancre (même page) |  |
| `#main` | ✅ Ancre (même page) |  |
| `/` | ✅ Interne valide |  |
| `/` | ✅ Interne valide |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/blog` | ✅ Interne valide |  |
| `/blog` | ✅ Interne valide |  |
| `/conseiller-gestion-patrimoine-independant` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet#modele` | ✅ Ancre (autre page) |  |
| `/le-cabinet#partenaires` | ✅ Ancre (autre page) |  |
| `/mentions-legales` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /mentions-legales |
| `/particuliers` | ✅ Interne valide |  |
| `/particuliers` | ✅ Interne valide |  |
| `/politique-confidentialite` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /politique-confidentialite |
| `https://policies.google.com/privacy` | ⚪ Externe (non testé) |  |
| `https://privacy.microsoft.com/fr-fr/privacystatement` | ⚪ Externe (non testé) |  |
| `https://www.cnil.fr` | ⚪ Externe (non testé) |  |
| `https://www.linkedin.com/company/108398030/` | ⚪ Externe (non testé) |  |
| `https://www.orias.fr/search?number=25006472` | ⚪ Externe (non testé) |  |
| `https://www.ovhcloud.com/fr/personal-data-protection/` | ⚪ Externe (non testé) |  |
| `mailto:direction@monte-cristo.net` | ⚪ mailto / tel |  |
| `mailto:direction@monte-cristo.net` | ⚪ mailto / tel |  |
| `mailto:direction@monte-cristo.net` | ⚪ mailto / tel |  |
| `mailto:kevin@monte-cristo.net` | ⚪ mailto / tel |  |
| `tel:+33629865266` | ⚪ mailto / tel |  |

### `simulateur-frais.html`
| Lien | Catégorie | Note |
|------|-----------|------|
| `#sim` | ✅ Ancre (même page) |  |
| `/` | ✅ Interne valide |  |
| `/` | ✅ Interne valide |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/#contact` | ✅ Ancre (autre page) |  |
| `/article-frais-bancaires` | ✅ Interne valide |  |
| `/blog` | ✅ Interne valide |  |
| `/blog` | ✅ Interne valide |  |
| `/conseiller-gestion-patrimoine-independant` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/entreprises` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet` | ✅ Interne valide |  |
| `/le-cabinet#modele` | ✅ Ancre (autre page) |  |
| `/le-cabinet#partenaires` | ✅ Ancre (autre page) |  |
| `/mentions-legales` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /mentions-legales |
| `/particuliers` | ✅ Interne valide |  |
| `/particuliers` | ✅ Interne valide |  |
| `/politique-confidentialite` | ✅ Interne valide | Page existante mais non déclarée dans pages.json : /politique-confidentialite |
| `https://www.linkedin.com/company/108398030/` | ⚪ Externe (non testé) |  |
| `https://www.orias.fr/search?number=25006472` | ⚪ Externe (non testé) |  |
| `mailto:kevin@monte-cristo.net` | ⚪ mailto / tel |  |
| `tel:+33629865266` | ⚪ mailto / tel |  |

---
*Rapport généré par `seo/scripts/check-links.py`*